...
用法:
    python -m scripts.init_data                    # 默认主板 500 只
    python -m scripts.init_data --limit 100        # 只灌 100 只
    python -m scripts.init_data --only-basic       # 只灌 stock_basic（快）
    python -m scripts.init_data --only-valuation   # 跳过 stock_basic，只灌估值
"""
import sys
import io
import time
...
from astock.data.db import get_conn, upsert_stock_basic, upsert_valuation
from astock.universe import filter_main_board
