...
> **2026-09-08 策略迁移**：新建交易实例与市场扫描已切换至 vn.py `Alpha101` 横截面信号；交易计划层继续负责限仓、整手和人工成交审计。本文其余 `screen.py`、旧多因子回测和模拟盘自动调仓内容为历史记录；相关自动入口已停用。
...
- **vn.py Alpha 策略已替换旧自动入口**：新候选基于 `Alpha101` 横截面表达式；旧每日评级和模拟盘日评不再自动成交，旧记录保留只读。
...
- 启动: `uvicorn astock.api.main:app --host 0.0.0.0 --port 8000`；文档: `/docs` (Swagger UI)
...
| 类别 | 文件 | 作用 |
|------|------|------|
| **数据层** | `astock/data/fetcher.py` | AKShare 多源数据获取（新浪/东财/同花顺/巨潮）|
| | `astock/data/db.py` | MySQL 访问层（连接 + UPSERT + 各表读写）|
| **策略/因子** | `astock/factors/compute.py` | 14 个因子计算 |
| | `astock/strategies.py` | 3 个策略 profile + 资金量自适应持仓数 |
| | `astock/selector.py` | 横截面打分排序 |
| | `astock/news_scorer.py` | 关键词消息面评分 |
| | `astock/pattern_recognizer.py` | 4 个 K 线形态识别 |
| | `astock/single_grader.py` | 单股 5 维度评级（astock/rate.py 用）|
| | `astock/grader.py` | 多维度评级（早期版本）|
| | `astock/universe.py` | 沪深主板筛选 |
| | `astock/financial_risk.py` | 财务暴雷、ST 和退市风险硬过滤 |
| | `astock/rating_store.py` | 全市场每日评级快照与趋势状态 |
| **入口** | `astock/screen.py` | 全市场选股 |
| | `astock/rate.py` | 单股深度评级 |
| | `examples/main.py` | 单股均线回测示例|
| | `examples/demo_backtest.py` | 离线回测演示 |
| | `scripts/query_backtest.py` | 历史回测查询工具 |
| | `astock/paper_engine.py` | **模拟盘核心引擎（新）** |
| | `astock/paper.py` | 模拟盘 CLI |
| | `astock/daily_runner.py` | **每日运行器**（全市场评级、持仓日评、择优替换；`run_all` 供调度&CLI 共用）|
| | `astock/api/scheduler.py` | **APScheduler 定时任务**（18:00 全市场评级+模拟盘 / 盘中10min监控 / 18:30 持仓分析 / 19:00 自选股邮件）|
| **自选股** | `astock/watchlist.py` | 自选股 CRUD、分组和评级策略 |
| | `astock/watchlist_analyzer.py` | 自选股评级变化、行情和消息汇总邮件 |
| **实盘持仓(新)** | `astock/real_holding.py` | 实盘持仓 CRUD（与模拟盘隔离，表自动建）|
| | `astock/holding_analyzer.py` | 盘后全方位分析（5维评级+新闻/公告/研报+涨跌 → HTML 报告）|
| | `astock/notify.py` | SMTP 邮件（配置存 DB `notify_config`，前端录入）|
| | `astock/api/routes/holdings.py` | 持仓 CRUD + 分析触发(SSE)|
| | `astock/api/routes/notify.py` | SMTP 配置 + 测试邮件接口 |
| **回测/验证** | `astock/backtest_simple.py` | 基础回测（IC + 止损 + DB 写入）|
| | `astock/backtest_rolling.py` | 动态滚动调权重 |
| | `astock/walk_forward.py` | 样本外验证 |
| | `astock/multi_window.py` | 多窗口验证 |
| **初始化** | `scripts/init_data.py` | 数据库初始化 |
| | `scripts/migrate_cache_to_db.py` | CSV cache 迁移 |
| | `sql/schema.sql` | 17 张表 schema |
| **测试** | `tests/test_mock.py`, `tests/test_rating.py` | mock 数据验证 |
...
| `backtest_run` 等 (4 张) | 累积中 | ✅ **每次 astock/backtest_simple.py 自动写入** |
| `paper_*` (5 张) | ✅ 累积中 | **Phase 2 MVP 已上线** |

### 3.3 14 个因子（5 维度）

| 维度 | 因子 | astock/screen.py | astock/rate.py |
|------|------|:---:|:---:|
...
```bash
git pull
POST /api/market-scans                                     # vn.py Alpha101 候选池
POST /api/trade-runs/{run_id}/plans/generate               # 生成待确认计划
```

### 回测验证
```bash
POST /api/vnpy/backtests                                   # vn.py Alpha 回测

# 查询历史回测（DB 自动存档）
python -m scripts.query_backtest                       # 列出最近 20 次
python -m scripts.query_backtest --run 5               # 查看第 5 次详情
python -m scripts.query_backtest --compare 3 5 7       # 对比多次
```
...
```bash
# 创建账户
python -m astock.paper create --name "swing-A" --capital 100000 --strategy swing
python -m astock.paper list

# 自动调仓（一键完成：全市场评级 + 持仓日评 + 择优替换 + 快照）
python -m astock.paper auto-rebalance --account 1 --limit 0

# 手动操作
python -m astock.paper buy --account 1 --code 600487.SH --qty 200 --price 75
python -m astock.paper stoploss --account 1
python -m astock.paper snapshot --account 1

# 查询
python -m astock.paper positions --account 1
python -m astock.paper trades --account 1
python -m astock.paper report --account 1

# ⭐ 每日运行器（CLI；正常由 APScheduler 自动跑，无需手动）
python -m astock.daily_runner                        # 跑今天，所有活跃账户
python -m astock.daily_runner --date 20260513        # 历史复盘
python -m astock.daily_runner --account 1            # 只跑某个账户
python -m astock.daily_runner --dry-run              # 预览不执行
...
### 数据初始化（一次性）
```bash
python -m scripts.migrate_cache_to_db                 # CSV cache → MySQL
python -m scripts.init_data --limit 2000              # 全市场 stock_basic + 估值
```
...
### 短期（完善 Tier 1/2）✅ **基本完成**
- [x] ~~接入财务数据 `market_financial`~~ ✅ fetcher DB 优先 + scripts/init_financial.py
...
### 中期（Phase 2 模拟盘） ✅ **基本完成**
- [x] ~~模拟账户管理~~ ✅ astock/paper_engine.py
- [x] ~~持仓跟踪 + 止损监控~~ ✅ check_stoploss
- [x] ~~每日复盘报告~~ ✅ daily_report
- [x] ~~自动调仓（嵌入 screen 选股）~~ ✅ astock/paper.py auto-rebalance
- [x] ~~多策略账户并行~~ ✅ 已建 short/swing/trend 三账户
- [x] ~~每日 cron 入口~~ ✅ astock/daily_runner.py（`run_all` 可编程入口）
...
### 实盘持仓分析（与模拟盘隔离）✅ **新上线**
- [x] ~~实盘持仓 CRUD~~ ✅ astock/real_holding.py（前端录入，表自动建）
- [x] ~~盘后全方位分析~~ ✅ astock/holding_analyzer.py（5 维评级 + 当天新闻/公告/研报 + 涨跌浮盈）
- [x] ~~邮件推送~~ ✅ astock/notify.py（SMTP，配置存 DB 由前端录入）+ 每交易日 18:30 自动发
...
- 想看**最新策略表现** → 此文档"3.4"
- 想看**所有可用命令** → 此文档"四"
- 想看**因子定义** → `astock/vnpy_runtime/vnpy/vnpy/alpha/dataset/datasets/alpha_101.py`
- 想看**候选生成** → `astock/vnpy_runtime/signals.py`
- 想看**组合规则** → `astock/vnpy_runtime/vnpy/vnpy/alpha/strategy/strategies/equity_demo_strategy.py`
- 想看**数据库表结构** → `sql/schema.sql`
- 想改 PE 等评分阈值 → `astock/single_grader.py` 顶部 `score_pe` / `score_pb` 等函数
- 想改关键词词典 → `astock/news_scorer.py` 顶部 `TITLE_KEYWORDS`
- 想加新因子 → 在 vn.py Alpha 数据集上新增表达式，并完成无未来数据回测后发布新策略版本
