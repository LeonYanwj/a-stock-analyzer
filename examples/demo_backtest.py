...
用法:
    python -m examples.demo_backtest                    # 默认 MA5xMA20
    python -m examples.demo_backtest --short 20 --long 60
    python -m examples.demo_backtest --code 000001.SZ --short 10 --long 30
"""
import sys
import io
import os
...
from astock.analysis.indicators import add_all_indicators
from astock.strategy.ma_cross import MACrossStrategy
...
def find_cache_csv(ts_code: str = "000001.SZ") -> str:
    ...
    if not matches:
        raise FileNotFoundError(
            f"找不到 {ts_code} 的缓存。先在项目根目录运行 python -m examples.main 或 python -m astock.screen 拉数据，"
            f"或把已有 CSV 放进 cache/。"
        )
