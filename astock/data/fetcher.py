...
class DataFetcher:
    ...
    def get_stock_financial_abstract(self, ts_code) -> pd.DataFrame:
        ...
        # 写回 DB（不影响返回，silent）
        try:
            from astock.data.db import get_conn, upsert_df
            db_df = df.copy()
    ...
    def get_stock_indicator(self, ts_code) -> pd.DataFrame:
        ...
        try:
            from astock.data.db import get_conn, upsert_valuation
            df_db = df.copy()
    ...
    def get_daily(self, ts_code, start_date, end_date, adjust="qfq", source="sina"):
        ...
        try:
            from astock.data.db import get_conn, query_daily, query_daily_coverage
        ...
        # 写回 DB（不影响返回，失败 silent）
        try:
            from astock.data.db import get_conn, upsert_daily
            with get_conn() as conn:
    ...
    def get_etf_daily(self, ts_code, start_date, end_date) -> pd.DataFrame:
        ...
        try:
            from astock.data.db import get_conn, upsert_etf_daily
            with get_conn() as conn:
