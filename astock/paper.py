...
用法:
    ...
    python -m astock.paper list                                              # 列出所有账户
    python -m astock.paper positions --account 1                             # 看持仓
    python -m astock.paper trades --account 1 [--limit 20]                   # 看成交
    python -m astock.paper buy  --account 1 --code 600487 --qty 200 --price 18.5 [--date 20260518]
    python -m astock.paper sell --account 1 --code 600487 --qty 200 --price 19.2
    python -m astock.paper rebalance --account 1 --picks 600487 002028 000001 --date 20260518
    python -m astock.paper stoploss --account 1 [--date 20260518]            # 触发止损检查
    python -m astock.paper snapshot --account 1 [--date 20260518]            # 存今日权益快照
    python -m astock.paper report --account 1 [--date 20260518]              # 生成复盘报告
"""
import sys
import io
import argparse
...
def cmd_auto_rebalance(args):
    """旧模拟盘自动调仓已停用。"""
    print("旧 screen.py 自动调仓已由 vn.py Alpha 策略替换。")
    print("请通过市场扫描与交易实例生成 vn.py Alpha 计划：")
    print("  POST /api/market-scans  →  POST /api/trade-runs/{run_id}/plans/generate")
