...
启动:
    uvicorn astock.api.main:app --host 0.0.0.0 --port 8000 --reload
...
from astock.api import errors as api_errors
from astock.api import scheduler as sched
from astock.api import auth
from astock.api.routes import (accounts, screen, rate, backtest, stocks, tasks,
...
app = FastAPI(
    title="A 股量化系统 API",
    description="vn.py Alpha 选股、交易计划、人工成交回填与数据查询",
    version="0.4.0",
    lifespan=lifespan,
)
...
def root():
    return {
        ...
        "docs": "/docs",
        "endpoints": {
            "accounts":  "/api/accounts（旧模拟盘查询；自动调仓已停用）",
            "screen":    "/api/screen（旧策略入口已停用）",
            "rate":      "/api/rate/{code}",
            "backtest":  "/api/backtest（旧回测查询；新回测使用 /api/vnpy/backtests）",
            "stocks":    "/api/stocks",
