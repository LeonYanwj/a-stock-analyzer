...
from astock.api.schemas import (AccountSummary, AccountHistoryRow, PositionRow,
                          TradeRow, EquityPoint)
from astock.api.errors import NotFound, BadRequest
from astock.api import tasks as task_mgr
...
def _do_auto_rebalance(task, account_id: int, limit: int, asof, enable_news: bool):
    ...
    task.report(100, "旧策略自动调仓已停用")
    return {
        "account_id": account_id,
        "skipped": True,
        "reason": "旧 screen.py 策略已由 vn.py Alpha 链路替换；请使用 /api/market-scans 和 /api/trade-runs。",
    }
...
def auto_rebalance_async(
    ...
    """旧自动调仓兼容入口：不会再生成旧策略成交。

    流程：
    请使用 vn.py Alpha 市场扫描与交易实例计划接口。
    """
    acc = eng.get_account(account_id)
    if acc is None:
        raise NotFound(f"账户 {account_id} 不存在", code="ACCOUNT_NOT_FOUND")
...
def _do_daily_run(task, account_id: int, asof, limit: int, dry_run: bool):
    ...
    """
    from astock.daily_runner import run_one_account
    import io
    import contextlib
...
def daily_run_async(
    ...
    等价于命令行：
        python -m astock.daily_runner --account {id} --date YYYYMMDD
