...
from astock.data.db import get_conn, list_backtest_runs, get_backtest_detail
from astock.api.schemas import BacktestRun
from astock.api.errors import NotFound, BadRequest
from astock.api import tasks as task_mgr
from astock.strategies import list_strategies
...
def _do_run_backtest(task, strategy: str, months: int, limit: int,
    ...
    """
    script = PROJECT_ROOT / "astock" / "backtest_simple.py"
    if not os.path.exists(script):
        raise NotFound(f"找不到回测脚本：{script}", code="BACKTEST_SCRIPT_MISSING")

    cmd = [sys.executable, "-m", "astock.backtest_simple",
           "--strategy", strategy,
           "--months", str(months),
           "--limit", str(limit),
...
def run_backtest_async(
    ...
    """
    raise BadRequest("旧多因子回测已停用；请使用 POST /api/vnpy/backtests 运行 vn.py Alpha 回测。",
                     code="LEGACY_STRATEGY_RETIRED")
