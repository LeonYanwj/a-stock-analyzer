...
def run_trade_run_plans(plan_window, now=None):
    ...
    from astock.api.routes.trade_runs import get_service
...
def run_daily_job(trade_date=None):
    ...
    import astock.daily_runner as daily_runner
...
def run_intraday_monitor():
    ...
    只评估持仓、只卖不买（买入放收盘）。
    """
    from astock.strategies import get_intraday_minutes
    now = datetime.now()
...
def run_holding_report():
    ...
    try:
        import astock.holding_analyzer as holding_analyzer
...
def run_watchlist_report():
    ...
    if not is_trade_day(now.date()):
        ...
        return
    try:
        import astock.watchlist_analyzer as watchlist_analyzer
