...
class SqliteTradeRunRepository:
    ...
    def initialize(self):
        ...
        for code, name, desc, fingerprint in seeds:
            ...
            self.conn.execute(
                "INSERT OR IGNORE INTO strategy_version(strategy_code,version_no,algorithm_fingerprint,config_json,signal_source,created_at) VALUES (?,?,?,?,?,?)",
                (code, 3, f"vnpy-alpha101-{code}-v1", '{"engine":"Alpha101","signal_mode":"cross_sectional_rank"}', "vnpy", now),
            )
        self.conn.commit()
    ...
    def initialize(self):
        ...
        with self.transaction():
            for code, name, description, fingerprint in [
                ...
                self.conn.execute("INSERT IGNORE INTO strategy_version(strategy_code,version_no,algorithm_fingerprint,config_json,signal_source,created_at) VALUES (?,?,?,JSON_OBJECT('engine','Alpha101','signal_mode','cross_sectional_rank'),?,?)", (code, 3, f"vnpy-alpha101-{code}-v1", "vnpy", now))
