...
from .models import (ASSET_TYPES, CREATABLE_SHADOW_SIGNAL_SOURCES, CREATABLE_SIGNAL_SOURCES,
                     PLAN_WINDOWS, RUNNABLE_STATUSES, SIGNAL_SOURCES,
...
class TradeRunService:
    ...
    def create_run(self, name, strategy_code, capital, max_position_pct, asset_types,
        ...
        signal_source = signal_source or "vnpy"
        require(signal_source in CREATABLE_SIGNAL_SOURCES, "SIGNAL_SOURCE_REQUIRED", "新交易实例只能使用 vnpy Alpha 信号体系")
        shadow_signal_source = shadow_signal_source or "vnpy_reference"
        require(shadow_signal_source in CREATABLE_SHADOW_SIGNAL_SOURCES,
                "INVALID_SHADOW_SIGNAL_SOURCE", "影子体系必须是与主体系不同的 vnpy_reference")
    ...
    def create_plan(self, run_id, ts_code, asset_type, side, suggested_qty, reference_price,
        ...
        with self.repo.transaction():
            ...
            signal_source = signal_source or run.get("primary_signal_source", "vnpy")
            require(signal_source in SIGNAL_SOURCES, "INVALID_SIGNAL_SOURCE", "不支持的信号体系")
