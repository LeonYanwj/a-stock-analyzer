...
def rebalance_by_signal(account_id: int, trade_date, top_n_target: int = None,
    ...
    """
    return _legacy_strategy_retired()

    from astock.screen import screen_market
    from astock.strategies import calc_optimal_top_n, get_keep_buffer
...
def rebalance_by_rating(account_id: int, trade_date, ratings: pd.DataFrame,
    ...
    from astock.financial_risk import assess_stock_eligibility
    from astock.rating_store import get_previous_rating
    from astock.rating_rules import rating_exit_reason
    from astock.strategies import calc_optimal_top_n
...
def _legacy_strategy_retired() -> dict:
    """阻止旧因子策略再对模拟账户产生自动成交。"""
    return {
        "error": "legacy_strategy_retired",
        "message": "旧 screen.py 多因子自动调仓已停用；请通过 vn.py Alpha 市场扫描和交易实例生成计划。",
    }
# -------------------- 止损 --------------------
