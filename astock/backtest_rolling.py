...
用法:
    python -m astock.backtest_rolling --start-year 2022 --end-year 2024 --limit 500
"""
import sys
import io
import os
...
from astock.strategies import (get_factor_weights, calc_optimal_top_n,
                        warn_if_capital_too_small)
from astock.backtest_simple import (
    TECH_ONLY_FACTORS, COMMISSION_TWO_WAY,
    _filter_tech_weights, get_rebal_dates,
    fetch_all_history, calc_metrics, fetch_benchmark,
)
from astock.walk_forward import adjust_weights_by_ic
...
def main():
    ...
    # ---- 1. 静态基线（不调权重）----
    print(f"\n[1] 静态基线（不调权重）...")
    from astock.walk_forward import run_period_backtest
    ...
    from astock.backtest_simple import save_backtest_to_db
